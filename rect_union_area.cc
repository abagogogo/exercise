...
bool cut_greater(const Cut& c1, const Cut& c2) {
    ...
    else if (c1.x == c2.x) return !c1.leaving;
    else return false;
}
...
int rects_coverage(vector<Rect> &rects) {
    ...
    // Set up a priority_queue to scan rectangles from left to right, leaving to entering.
    priority_queue<Cut, vector<Cut>, 
        std::function<bool(const Cut&, const Cut &)>> cuts(cut_greater);
    for (auto &rect : rects) {
        cuts.push({rect.upper_left.x, false, {rect.upper_left.y, rect.lower_right.y}});
        cuts.push({rect.lower_right.x, true, {rect.upper_left.y, rect.lower_right.y}});
    }

   
    // For each cut, if its x-axis is changed, update area and restart one.
    // if the cut is right side of a rectangle, remove the line from active line segments.
    // if the cut is left side of a rectangle, add the line to active line segments.
